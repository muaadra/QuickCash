...
public class SubscriptionsRVListAdapter extends RecyclerView.Adapter<SubscriptionsRVListAdapter.ListItem> {
    private String[]  listItems;
    List<Boolean> presets;
    /**
     * constructor
     * @param listItems array of TaskPost objects to show on the list
     */
    public SubscriptionsRVListAdapter(String[]  listItems, List<Boolean> presets) {
    ...
    public void onBindViewHolder(ListItem listItem, int position) {
        ...
        if(position < presets.size()){
            ((CheckBox)listItem.mainView.findViewById(R.id.listCheckBox))
                    .setChecked(presets.get(position));
        }
